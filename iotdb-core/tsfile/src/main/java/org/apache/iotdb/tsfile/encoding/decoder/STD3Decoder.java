 ...
 */
public abstract class STD3Decoder extends Decoder {
    ...
    public static class LongSTDDecoder extends STD3Decoder {
...
//            return firstValue;
            ...
            encodingLength = (seasonalBlockBufferLength + 7) / 8 + (blockBufferLength + 7) / 8;
            ...
            for (int i = 0; i < period; i++) {
                ...
                if (pos % 8 == 0) {
                    for (int j = 0; j < (width - 1) / 8; j++) {
                        value |= (long) (stdBuf[pos / 8 + j] & 0xFF) << (j * 8);
                    }
                    value |= (stdBuf[(pos + width - 1) / 8] & (0xFF & ~(0xFFL << (width % 8)))) << ((width - 1) / 8 * 8);
                } else {
                    value |= (stdBuf[pos / 8] & (0xFF & (0xFFL << (pos % 8))));
                    for (int j = 1; j < (width + pos % 8 - 1) / 8; j++) {
                        value |= (long) (stdBuf[pos / 8 + j] & 0xFF) << (j * 8);
                    }
                    value |= (stdBuf[(pos + width - 1) / 8] & (0xFF & ~(0xFFL << (pos + width % 8)))) << ((width + pos % 8 - 1) / 8 * 8);
                }
        ...
        private void readVarIntPack() {
            long value;
            int pos = (seasonalBlockBufferLength + 7) / 8 * 8;
            int width;
            for (int i = 0; i < packNum; i++) {
