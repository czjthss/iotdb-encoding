 ...
 */
public class LongRleEncoder2 extends RleEncoder2<Long> {
     ...
     */
    public LongRleEncoder2() {
        super();
        bufferedValues = new Long[TSFileConfig.RLE_MIN_REPEATED_NUM];
        preValue = (long) 0;
        values = new Long[config.getStdBlockSize()];
    }
    ...
    public void encode(long value, ByteArrayOutputStream out) {
        ...
        try {
            if (valuesIndex == config.getStdBlockSize()) {
                flush(out);
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private int getLongMaxBitWidth(Long[] list) {
        int max = 1;
        for (long num : list) {
    ...
    public void flush(ByteArrayOutputStream out) throws IOException {
        ...
        this.bitWidth = getLongMaxBitWidth(values);
    ...
    protected void writeRleRun() throws IOException {
        ...
        ReadWriteForEncodingUtils.writeUnsignedVarInt(repeatCount << 1, byteCache);  // writeUnsignedVarInt
    ...
    public long getMaxByteSize() {
        ...
        int groupNum = (values.length / 8 + 1) / 63 + 1;
        return (long) 8 + groupNum * 5 + values.length * 8L;
    }
}
