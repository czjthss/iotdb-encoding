 ...
 */
public class IntRleEncoder2 extends RleEncoder2<Integer> {
    ...
    public IntRleEncoder2() {
        super();
        bufferedValues = new Integer[TSFileConfig.RLE_MIN_REPEATED_NUM];
        preValue = 0;
        values = new Integer[config.getStdBlockSize()];
    }
    ...
    public int getIntMaxBitWidth(Integer[] list) {
        int max = 1;
//        for (int num : list) {
        for (int idx = 0; idx < valuesIndex; idx++) {
            int bitWidth = 32 - Integer.numberOfLeadingZeros(list[idx]);
    ...
    public long getMaxByteSize() {
        ...
        return (long) 8 + groupNum * 5 + values.length * 4;
    }
}
